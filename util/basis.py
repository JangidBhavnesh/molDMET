...
from mrh.util.la import is_matrix_zero, is_matrix_eye, is_matrix_idempotent, matrix_eigen_control_options, \
    matrix_svd_control_options, align_vecs
...
def assert_vector_statelist(test_vector, max_element=None, max_length=None, min_length=0):
    ...
    err_str = "vector not 1d array of unique nonnegative integers with {0} <= length <= {1} and maximum element < {2}\n({3})".format(
        min_length, max_length, max_element, test_vector)
...
def represent_operator_in_basis(braOket, bra1_basis=None, ket1_basis=None, bra2_basis=None, ket2_basis=None):
    ...
    if len(braOket.shape) == 2:
        ...
        raise ValueError(
            "Only one- and two-body operators (two- and four-index arrays) or pairs of one-body operators supported")
...
def get_overlapping_states(bra_basis, ket_basis, across_operator=None, inner_symmetry=None, outer_symmetry=(None, None),
                           enforce_symmetry=False,
                           max_nrvecs=0, max_nlvecs=0, num_zero_atol=params.num_zero_atol, only_nonzero_vals=True,
                           full_matrices=False):
    c2p = np.asarray(bra_basis)
    c2q = np.asarray(ket_basis)
    ...
    assert (c2p.shape[1] <= c2p.shape[
        0]), "you need to give the first state in a complete basis (c2p). Did you accidentally transpose it?"
    assert (c2q.shape[1] <= c2q.shape[
        0]), "you need to give the second state in a complete basis (c2q). Did you accidentally transpose it?"
    ...
    if np.any(across_operator):
        assert (c2p.shape[0] == cOc.shape[0] and c2p.shape[0] == cOc.shape[
            1]), "when specifying an across_operator, it's dimensions need to be the same as the external basis"
    get_labels = (not (inner_symmetry is None)) or (not (outer_symmetry[0] is None)) or (
        not (outer_symmetry[1] is None))
    ...
    try:
        rets = matrix_svd_control_options(cOc, lspace=c2p, rspace=c2q, full_matrices=full_matrices,
                                          ...
                                          sort_vecs=-1, only_nonzero_vals=only_nonzero_vals,
                                          num_zero_atol=num_zero_atol)
        ...
        rets = matrix_eigen_control_options(proj_l, subspace=c2q, symmetry=r_symmetry, strong_symm=enforce_symmetry,
                                            sort_vecs=-1,
        ...
        rets = matrix_eigen_control_options(proj_r, subspace=c2p, symmetry=l_symmetry, strong_symm=enforce_symmetry,
                                            sort_vecs=-1,
                                            only_nonzero_vals=False, num_zero_atol=num_zero_atol)
        evals_l, c2l = rets[:2]
...
def orthonormalize_a_basis(overlapping_basis, ovlp=1, num_zero_atol=params.num_zero_ltol, symmetry=None,
                           enforce_symmetry=False):
    ...
    if enforce_symmetry:
        ...
        for c2s in symmetry:
            ...
            s2n = orthonormalize_a_basis(s2b, ovlp=sOs, num_zero_atol=num_zero_atol, symmetry=None,
                                         enforce_symmetry=False)
...
def get_states_from_projector(the_projector, num_zero_atol=params.num_zero_atol):
    ...
    assert (np.allclose(proj_cc, proj_cc.H)), "projector must be hermitian\n" + str(
        np.linalg.norm(proj_cc - proj_cc.conjugate().T))
    assert (is_matrix_idempotent(proj_cc)), "projector must be idempotent\n" + str(
        np.linalg.norm((proj_cc @ proj_cc) - proj_cc))
...
def get_complementary_states(incomplete_basis, already_complete_warning=True, atol=params.num_zero_atol, symmetry=None,
                             enforce_symmetry=False):
    ...
    if enforce_symmetry:
        ...
        for c2s in symmetry:
            ...
            if not count_linind_states(s2b):
                ...
                continue
            s2p = get_complementary_states(s2b, atol=atol, already_complete_warning=False, symmetry=None,
                                           enforce_symmetry=False)
    ...
    assert (are_bases_equivalent(Q[:, :nbas], orthonormal_basis)), 'basis overlap = {}'.format(
        measure_basis_olap(Q[:, :nbas], orthonormal_basis))
    assert (are_bases_orthogonal(Q[:, nbas:], orthonormal_basis)), 'basis overlap = {}'.format(
        measure_basis_olap(Q[:, nbas:], orthonormal_basis))
    return orthonormalize_a_basis(Q[:, nbas:])


def get_complete_basis(incomplete_basis, symmetry=None, enforce_symmetry=False):
    complementary_states = get_complementary_states(incomplete_basis, already_complete_warning=False, symmetry=symmetry,
...
def is_operator_block_adapted(the_operator, the_blocks, tol=params.num_zero_atol):
    ...
    if isinstance(the_blocks[0], np.ndarray):
        ...
        assert (is_basis_orthonormal_and_complete(umat)), 'Symmetry blocks must be orthonormal and complete, {}'.format(
            len(the_blocks))
...
def are_states_block_adapted(the_basis, the_blocks, atol=params.num_zero_atol, rtol=params.num_zero_rtol):
    ...
    for blk in the_blocks:
        ...
        if not (np.all(np.logical_or(is_symm < rtol,
                                     # Better alternative to np.isclose (is_symm, 0) b/c of the way the latter works
...
def assign_blocks(the_basis, the_blocks, atol=params.num_zero_atol, rtol=params.num_zero_rtol):
    assert (is_subspace_block_adapted(the_basis, the_blocks,
                                      tol=atol)), 'Basis space must be block-adapted before assigning states'
    ...
    for idx, blk in enumerate(the_blocks):
        ...
        assert (
            check), 'Basis states must be individually block-adapted before being assigned (is_symm = {} for label {})'.format(
            is_symm, idx)
...
def symmetrize_basis(the_basis, the_blocks, sorting_metric=None, sort_vecs=1, do_eigh_metric=True,
                     check_metric_block_adapted=True, atol=params.num_zero_atol, rtol=params.num_zero_rtol):
    atol_scl = atol * the_basis.shape[0]
    rtol_scl = rtol * the_basis.shape[0]
    if the_blocks is None: the_blocks = [np.eye(the_basis.shape[0])]
    assert (is_subspace_block_adapted(the_basis, the_blocks,
                                      tol=atol)), 'Basis space must be block-adapted before blockifying states'
    ...
    if sorting_metric is None:
        ...
        if sorting_metric.shape[0] == the_basis.shape[0]:
            ...
            assert (sorting_metric.shape[0] == the_basis.shape[
                1]), 'The sorting metric must be in either the row or column basis of the orbital matrix that is being symmetrized'
        ...
        metric_evals, evecs, labels = matrix_eigen_control_options(metric_symm, symm_blocks=labels, sort_vecs=sort_vecs,
                                                                   only_nonzero_vals=False, num_zero_atol=atol)
        symmetrized_basis = symmetrized_basis @ evecs
        return symmetrized_basis, labels, metric_evals


def align_states(unaligned_states, the_blocks, sorting_metric=None, sort_vecs=1, atol=params.num_zero_atol,
    ...
    assert (
        is_basis_orthonormal_and_complete(block_umat)), 'Symmetry blocks must be orthonormal and complete, {}'.format(
        len(the_blocks))

    if sorting_metric is None: sorting_metric = np.diag(np.arange(unaligned_states.shape[1]))
    if sorting_metric.shape[0] == unaligned_states.shape[1]: sorting_metric = represent_operator_in_basis(
        sorting_metric, unaligned_states.conjugate().T)
...
def eigen_weaksymm(the_matrix, the_blocks, subspace=None, sort_vecs=1, only_nonzero_vals=False,
                   atol=params.num_zero_atol, rtol=params.num_zero_rtol):
    ...
    evals, evecs = matrix_eigen_control_options(subspace_matrix, symm_blocks=None, sort_vecs=sort_vecs,
...
def get_subspace_symmetry_blocks(the_subspace, the_blocks, atol=params.num_zero_atol, rtol=params.num_zero_rtol):
    ...
    for idx, c2s in enumerate(the_blocks):
        ...
        svals, p2s = matrix_svd_control_options(s2p, rspace=remaining_space, only_nonzero_vals=True, full_matrices=True,
                                                sort_vecs=-1, num_zero_atol=rtol)[1:3]
        wgts = (p2s * p2s).sum(0)[:len(svals)]
        assert (np.all(np.isclose(wgts, 1, atol=atol,
                                  rtol=rtol))), 'Subspace may not be symmetry-adapted: svals for {}th block: {}'.format(
            idx, wgts)
