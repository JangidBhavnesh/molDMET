...
from mrh.util.basis import represent_operator_in_basis, project_operator_into_subspace, orthonormalize_a_basis, \
    get_complete_basis, get_complementary_states
from mrh.util.basis import is_basis_orthonormal, get_overlapping_states, is_basis_orthonormal_and_complete, \
    compute_nelec_in_subspace, get_subspace_symmetry_blocks
...
def solve(frag, guess_1RDM, chempot_imp):
    ...
    if len(frag.imp_cache) == 2:
        ...
        print(
            "Projecting stored amos (frag.loc2amo; spanning {} electrons) onto the impurity basis and filling the remainder with default guess".format(
                nelec_imp_guess))
        imp2mo, my_occ = project_amo_manually(frag.loc2imp, frag.loc2amo, fock_imp, norbs_cmo_guess,
    ...
    if len(frag.imp_cache) != 2 and frag.ci_as is not None:
        ...
        if (svals.size == norbs_amo):
            print("Loading ci guess despite shifted impurity orbitals; singular value error sum: {}".format(
                np.sum(svals - metric)))
            ...
            print("Discarding stored ci guess because orbitals are too different (missing {} nonzero svals)".format(
                norbs_amo - svals.size))

    # Symmetry align if possible
    imp2unac = \
        frag.align_imporbs_symm(np.append(imp2mo[:, :norbs_cmo], imp2mo[:, norbs_occ:], axis=1),
    ...
    if frag.mfmo_printed == False and frag.ints.mol.verbose:
        ...
        frag.mfmo_printed = True
            ...
            frag.active_orb_list) > 0:  # This is done AFTER everything else so that the _mfmorb.molden always has consistent ordering
    ...
    if frag.enforce_symmetry: imp2mo = lib.tag_array(imp2mo,
                                                     orbsym=label_orb_symm(mol, mol.irrep_id, mol.symm_orb, imp2mo,
                                                                           s=mf.get_ovlp(), check=False))
    ...
    mc.mo_coeff[:, :norbs_cmo] = \
        frag.align_imporbs_symm(mc.mo_coeff[:, :norbs_cmo], sorting_metric=fock_imp, sort_vecs=1,
                                orbital_type='optimized inactive', mol=mol)[0]
    mc.mo_coeff[:, norbs_cmo:norbs_occ], umat = frag.align_imporbs_symm(mc.mo_coeff[:, norbs_cmo:norbs_occ],
                                                                        sorting_metric=oneRDM_amo, sort_vecs=-1,
                                                                        orbital_type='optimized active', mol=mol)
    mc.mo_coeff[:, norbs_occ:] = \
        frag.align_imporbs_symm(mc.mo_coeff[:, norbs_occ:], sorting_metric=fock_imp, sort_vecs=1,
                                orbital_type='optimized external', mol=mol)[0]
    if frag.enforce_symmetry:
    ...
    print('Impurity CASSCF energy (incl chempot): {}; spin multiplicity: {}; time to solve: {}'.format(E_CASSCF,
                                                                                                       spin_square(mc)[
                                                                                                           1],
                                                                                                       t_end - t_start))
...
def project_amo_manually(loc2imp, loc2gamo, fock_mf, norbs_cmo, dm=None):
    ...
    if dm is not None:
        ...
        evals, evecs = matrix_eigen_control_options(represent_operator_in_basis(dm, loc2amo), sort_vecs=-1,
