...
from molDMET.util.la import is_matrix_zero, is_matrix_eye, is_matrix_idempotent, matrix_eigen_control_options, \
    matrix_svd_control_options, align_vecs, assert_matrix_square
from molDMET.util.la import *
from molDMET.util.basis import represent_operator_in_basis, measure_basis_nonorthonormality, get_overlapping_states, orthonormalize_a_basis, get_complementary_states
...
def get_1RDM_from_OEI_in_subspace(one_electron_hamiltonian, subspace_basis, nocc_subspace):
...
def Schmidt_decompose_1RDM(the_1RDM, loc2frag, norbs_bath_max, symmetry=None, fock_helper=None, enforce_symmetry=False,
                           bath_tol=params.num_zero_atol, num_zero_atol=params.num_zero_atol,
                           num_zero_rtol=params.num_zero_rtol):

    get_labels = not (symmetry is None)
    ...
    assert (norbs_frag <= norbs_tot == loc2frag.shape[0])
    ...
    rets = get_overlapping_states(loc2env, loc2frag, across_operator=the_1RDM, inner_symmetry=symmetry,
                                  outer_symmetry=(env_symm, frag_symm), enforce_symmetry=enforce_symmetry,
                                  only_nonzero_vals=True, full_matrices=True)
    loc2env, loc2frag, svals = rets[:3]
    if get_labels:
        env_labels, frag_labels = rets[3:]

    norbs_bath = len(svals)
    ...
    if norbs_ufrag > 0:
        ...
        rets = matrix_eigen_control_options(mat, subspace=loc2ufrag, symmetry=symmetry, strong_symm=enforce_symmetry,
    ...
    if norbs_core > 0:
        ...
        mat = the_1RDM if fock_helper is None else fock_helper
        rets = matrix_eigen_control_options(mat, subspace=loc2core, symmetry=symmetry, strong_symm=enforce_symmetry,
                                            sort_vecs=-1, only_nonzero_vals=False)
        loc2core = rets[1]
...
def Schmidt_decomposition_idempotent_wrapper(working_1RDM, loc2wfrag, norbs_bath_max, symmetry=None, fock_helper=None,
                                             enforce_symmetry=False,
                                             bath_tol=params.num_zero_atol, idempotize_thresh=0,
                                             num_zero_atol=params.num_zero_atol):
    norbs_tot = loc2wfrag.shape[0]
    norbs_wfrag = loc2wfrag.shape[1]
    loc2wemb, norbs_wbath, nelec_wimp, labels = Schmidt_decompose_1RDM(working_1RDM, loc2wfrag, norbs_bath_max,
                                                                       fock_helper=fock_helper,
                                                                       bath_tol=bath_tol, symmetry=symmetry,
    ...
    #print("Schmidt decomposition found {0} bath orbitals for this fragment, of an allowed total of {1}".format(
    #    norbs_wbath, norbs_bath_max))
    ...
    if norbs_wcore > 0:
        ...
        if abs(idempotize_thresh) > num_zero_atol:
            ...
            print("After attempting to idempotize the core (part of the putatively idempotent guide) 1RDM with a "
                "threshold of "+ "{0}, {1} electrons were found in the impurity".format(idempotize_thresh, nelec_wimp))
    if not np.isclose(nelec_wimp, round(nelec_wimp), atol=num_zero_atol, rtol=1e-5):
        raise RuntimeError(
            "Can't solve impurity problems without integer number of electrons! nelec_wimp={0}".format(nelec_wimp))
