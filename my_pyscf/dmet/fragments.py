...
from molDMET.my_pyscf.dmet import pyscf_rhf, pyscf_mp2, pyscf_cc, pyscf_casscf, qcdmethelper, pyscf_fci  # , chemps2
...
from molDMET.util.io import warnings
from molDMET.util.rdm import Schmidt_decomposition_idempotent_wrapper, idempotize_1RDM, get_1RDM_from_OEI, \
    get_2RDM_from_2CDM, get_2CDM_from_2RDM, Schmidt_decompose_1RDM
...
# Currently supports only one fragment but can be extended to more than one frag !!
def make_fragment_atom_list(ints, frag_atom_list, solver_name, bath_tol=1e-5, **kwargs):
    ...
    return fragment_object(ints, np.asarray(frag_orb_list), frag_atom_list, solver_name, bath_tol, **kwargs)
...
class fragment_object:
    ...
    def __init__(self, ints, frag_orb_list, frag_atom_list, solver_name, bath_tol=1e-5,
                 **kwargs):
        ...
        self.nelec_imp = 0
        self.norbs_bath_max = len(frag_orb_list)  # Why?
        self.solve_time = 0.0
        self.frag_name = 'None'
        ...
        if active_space:
            ...
            if not (len(self.active_space) == 2):
                raise RuntimeError(
                    "Active space {0} not usable; only CASSCF currently implemented".format(solver.active_space))
            self.imp_solver_longname += " with {0} electrons in {1} active-space orbitals".format(self.active_space[0],
                                                                                                  self.active_space[1])

        # Set up the main basis functions. Before any Schmidt decomposition all environment states are treated as
        # "core" self.loc2emb is always defined to have the norbs_frag fragment states, the norbs_bath bath states,
        # and the norbs_core core states in that order
        ...
        log = logger.new_logger(self, verbose=ints.mol.verbose)
        self.dump_flags(ints, frag_orb_list, frag_atom_list, log)

    def dump_flags(self, ints, frag_orb_list, frag_atom_list, verbose=None):
        log = logger.new_logger(self, verbose)
        log.info('')
        log.info('******** fragmentation flags ********')
        log.info("total orb: %s", self.norbs_tot)
        log.info("frag orb: %s", self.norbs_frag)
        log.info("nfrag atoms: %s", len(frag_atom_list))
        log.info("frag atom list\n{0}".format(
            [ints.mol.atom_symbol(atom) for atom in frag_atom_list]))
        log.info("frag orb list:\n{0}".format(frag_orb_list))
        log.info("High level solver: %s", self.imp_solver_method)
        return self
    ...
    def get_loc2canon_imp(self, oneRDM_loc=None, fock_loc=None):
        ...
        unac_ene, loc2unac_imp, unac_lbls = matrix_eigen_control_options(fock_loc, subspace=loc2unac_imp,
                                                                         symmetry=self.loc2symm,
                                                                         strong_symm=self.enforce_symmetry, sort_vecs=1,
                                                                         only_nonzero_vals=False)
        amo_occ, loc2amo_imp, amo_lbls = matrix_eigen_control_options(oneRDM_loc, subspace=self.loc2amo,
                                                                      symmetry=self.loc2symm,
                                                                      strong_symm=self.enforce_symmetry, sort_vecs=-1,
                                                                      only_nonzero_vals=False)
        ...
        loc2canon_imp = np.concatenate([loc2unac_imp[:, :norbs_inac], loc2amo_imp, loc2unac_imp[:, norbs_inac:]],
                                       axis=1)
    ...
    def get_loc2canon_core(self, all_frags, oneRDM_loc=None, fock_loc=None):
        ...
        unac_ene, loc2unac_core, unac_lbls = matrix_eigen_control_options(fock_loc, subspace=loc2unac_core,
                                                                          symmetry=self.loc2symm,
        ...
        amo_occ, loc2amo_core, amo_lbls = matrix_eigen_control_options(oneRDM_loc, subspace=loc2amo_core,
                                                                       symmetry=self.loc2symm,
                                                                       strong_symm=self.enforce_symmetry, sort_vecs=-1,
                                                                       only_nonzero_vals=False)
    ...
    def do_Schmidt(self, oneRDM_loc, all_frags, loc2wmcs):
        log = logger.new_logger(self, verbose=self.ints.mol.verbose)
        log.info("initiating schmidt decomposition")
        log.info("bath tol.: %s", self.bath_tol)
        log.info("max bath orb: %s", self.norbs_bath_max)

        self.imp_cache = []
        self.loc2emb, norbs_bath, self.nelec_imp, self.oneRDMfroz_loc, emb_labels = \
            Schmidt_decomposition_idempotent_wrapper(oneRDM_loc, self.loc2frag, self.norbs_bath_max,
                                                     symmetry=self.loc2symm, fock_helper=self.ints.activeFOCK,
                                                     enforce_symmetry=self.enforce_symmetry,
                                                     idempotize_thresh=self.idempotize_thresh, bath_tol=self.bath_tol,
        ...
        log.info("bath orb: %s", norbs_bath)
        log.info("core orb: %s", oneRDM_loc.shape[0] - self.nelec_imp)
        log.info("embedding ele: %s ", self.nelec_imp)
        log.info("embedding orb: %s ", self.norbs_imp)
        sys.stdout.flush()
        return self
    ...
    def get_quasifrag_ovlp(self, loc2frag, loc2wmcs, norbs_xtra):
        ...
        loc2qfrag, _, svals = get_overlapping_states(loc2wmcs, self.get_true_loc2frag(), inner_symmetry=self.loc2symm,
                                                     enforce_symmetry=self.enforce_symmetry)[:3]
        loc2qenv = get_complementary_states(loc2qfrag, already_complete_warning=False, symmetry=self.loc2symm,
                                            enforce_symmetry=self.enforce_symmetry)
        loc2wmas = get_complementary_states(loc2wmcs, already_complete_warning=False, symmetry=self.loc2symm,
                                            enforce_symmetry=self.enforce_symmetry)
        loc2p = orthonormalize_a_basis(np.concatenate([loc2frag, loc2qenv, loc2wmas], axis=1), symmetry=self.loc2symm,
                                       enforce_symmetry=self.enforce_symmetry)
        ...
        loc2qfrag, _, svals, qfrag_labels, _ = get_overlapping_states(loc2qfrag, self.get_true_loc2frag(),
                                                                      inner_symmetry=self.loc2symm,
                                                                      enforce_symmetry=self.enforce_symmetry,
        ...
        if (len(svals) > norbs_qfrag) and (norbs_qfrag > 0):
            ...
            if ndegen > 0:
                print(
                    "Warning: adding {} instead of {} quasi-fragment orbitals in order to avoid artificial symmetry breaking by adding only part of a degenerate manifold".format(
        ...
        if norbs_qfrag > 0:
            ...
            loc2wfrag, wfrag_labels = matrix_eigen_control_options(self.ints.activeFOCK, subspace=loc2wfrag,
                                                                   symmetry=self.loc2symm,
                                                                   strong_symm=self.enforce_symmetry,
                                                                   only_nonzero_vals=False, sort_vecs=1)[1:]
    ...
    def get_quasifrag_gradient(self, loc2frag, loc2wmcs, oneRDM_loc, norbs_xtra):
        ...
        loc2env = get_complementary_states(loc2frag, already_complete_warning=False, symmetry=self.loc2symm,
                                           enforce_symmetry=self.enforce_symmetry)
        loc2cenv = \
            get_overlapping_states(loc2wmcs, loc2env, inner_symmetry=self.loc2symm,
                                   enforce_symmetry=self.enforce_symmetry)[
                0]
        ...
        loc2qfrag, _, svals, qfrag_labels, _ = get_overlapping_states(loc2cenv, loc2amo, inner_symmetry=self.loc2symm,
                                                                      enforce_symmetry=self.enforce_symmetry,
                                                                      across_operator=grad, full_matrices=True,
                                                                      only_nonzero_vals=False)
        print("The gradient norm (outside of the fragment space) of the {} fragment is {}".format(self.frag_name,
                                                                                                  linalg.norm(svals)))
    ...
    def analyze_ao_imp(self, oneRDM_loc, loc2wmcs, norbs_bath_xtra):
        ...
        mo_occ, mo_evec = matrix_eigen_control_options(oneRDM_loc, sort_vecs=1, only_nonzero_vals=False,
                                                       subspace=loc2wmcs, symmetry=self.loc2symm,
        ...
        loc2virtbath, svals, _, virtbath_labels = get_overlapping_states(loc2ao, loc2virtunaccore,
                                                                         inner_symmetry=self.loc2symm,
                                                                         enforce_symmetry=self.enforce_symmetry)[1:]
        ...
        print(
            "Are my `virtual bath' orbitals unoccupied and currently in the core? check_nocc = {}, check_bath = {}".format(
                check_nocc, check_bath))
        ...
        print(("For this Schmidt decomposition, the impurity basis loses {} of {} atomic orbitals, accounted for by"
               " {} of {} virtual core orbitals").format(lost_aos, self.norbs_frag, aos_in_virt_core,
                                                         loc2virtbath.shape[1]))
        ...
        virtbathGocc, occ_labels = self.gradient_for_virtbath(loc2virtbath, oneRDM_loc, loc2wmcs,
                                                              fock=self.ints.activeFOCK)
        my_ene = -svals * svals
        my_occ = virtbathGocc.sum(1)
        if self.virtual_bath_gradient_svd:
            umat, svals_fock, vmat = matrix_svd_control_options(virtbathGocc, sort_vecs=-1, only_nonzero_vals=False,
                                                                lsymm=virtbath_labels, rsymm=occ_labels,
                                                                full_matrices=True, strong_symm=self.enforce_symmetry)[
                                     :3]
        ...
        if self.molden_missing_aos:
            ...
            molden.from_mo(self.ints.mol, self.filehead + self.frag_name + '_missing_AOs.molden', ao2molden, ene=my_ene,
                           occ=my_occ)
            self.molden_missing_aos = False
        return loc2virtbath
    ...
    def gradient_for_virtbath(self, loc2virtbath, oneRDM_loc, loc2wmcs, fock=None):
        loc2imp_unac = \
            get_overlapping_states(self.loc2imp, loc2wmcs, only_nonzero_vals=True, inner_symmetry=self.loc2symm,
                                   enforce_symmetry=self.enforce_symmetry)[0]
        evals, loc2no_unac, no_labels = matrix_eigen_control_options(oneRDM_loc, subspace=loc2imp_unac,
                                                                     symmetry=self.loc2symm, sort_vecs=-1,
    ...
    def test_Schmidt_basis_energy(self):
        ...
        print("CHECK ME: diff between self.ints.oneRDM_loc and self.oneRDM_loc: {}".format(
            linalg.norm(self.oneRDM_loc - self.ints.oneRDM_loc)))
        print("CHECK ME: diff between self.ints.oneSDM_loc and self.oneSDM_loc: {}".format(
            linalg.norm(self.oneSDM_loc - self.ints.oneSDM_loc)))
        print("CHECK ME: diff between projected rdm and self.oneRDMfroz_loc: {}".format(
            linalg.norm(q @ (dma + dmb) @ q - self.oneRDMfroz_loc)))
    ...
    def construct_impurity_hamiltonian(self, xtra_CONST=0.0):
        ...
        if self.imp_solver_name == "RHF" and self.quasidirect:
            ...
            # These two lines have memory handlement problem
            #vk_c = np.tensordot(eri, cdm, axes=((1, 2), (0, 1)))
            #vk_s = np.tensordot(eri, sdm, axes=((1, 2), (0, 1)))
            vk_c = lib.einsum('apqb,pq->ab', eri, cdm)
            vk_s = lib.einsum('apqb,pq->ab', eri, cdm)
            sie = self.E2_cum
            sie += np.tensordot(vj, cdm) / 2
            sie -= np.tensordot(vk_c, cdm) / 4
            sie -= np.tensordot(vk_s, sdm) / 4
        ...
        self.impham_built = True
        self.imp_solved = False
        print("Time in impurity Hamiltonian constructor: {:.8f} wall, {:.8f} clock".format(time.time() - w0,
    ...
    def test_impurity_hamiltonian_energy(self):
        ...
        print(
            "LASSCF energy total error in fragments.py (using stored density-matrix data, VALID FOR SINGLET ENVIRONMENT ONLY): {:.6e}".format(
                e_tot - self.ints.e_tot))
        print("CHECK ME: diff between stored E2_cum and recomputed E2_cum in this case: {}".format(
            self.E2_cum + sum(self.E2froz_tbc) - (v * self.twoCDMimp_amo).sum() / 2))
        ...
        if self.ci_as is not None:
            ...
            print(
                "LASSCF energy total error using ci vectors in fragments.py (VALID FOR SINGLET ENVIRONMENT ONLY): {:.6e}".format(
                    e_core + e_cas - self.ints.e_tot))
    ...
    def solve_excited_problem(self, chempot_frag,  method='adc(2)', nroots=1, ncvs=1, corehole=0):
        ...
        from molDMET.my_pyscf.dmet import pyscf_adc, pyscf_eomccsd

        if method == 'adc(2)' or method == 'adc(3)':
            pyscf_adc.solve(self, guess_1RDM, chempot_imp, nroots=nroots, ncvs=ncvs, corehole=corehole, method=method)
        elif method == 'IPCVSEOM':
            pyscf_eomccsd.solve(self, guess_1RDM, chempot_imp, nroots=nroots, ncvs=ncvs, corehole=corehole)
        else:
            raise NotImplementedError("{} is not available yet".format(method))
    ...
    def solve_impurity_problem(self, chempot_frag):
        ...
        #print("Impurity results for {0}: E_imp = {1}, E_frag = {2}, nelec_frag = {3}, S2_frag = {4}".format(
        #    self.frag_name,
    ...
    def analyze_gradient(self, oneRDM_loc=None, fock_loc=None):
        ''' Orbitals may be messed up by symmetry enforcement! I think symmetry enforcement may accidentally siwtch
        some impurity external orbitals and core external orbitals?'''
        ...
        print(
            "I think I have {} impurity inactive orbitals; occupancy error = {}, overlap error = {}".format(norbs_iinac,
                                                                                                            occ_err,
                                                                                                            olap_err))
        occ_err = linalg.norm(ino_occ[norbs_iinac:])
        olap_err = measure_basis_olap(iext2loc.conjugate().T, self.loc2core)
        print("I think I have {} impurity external orbitals; occupancy error = {}, overlap error = {}".format(
            len(ino_occ) - norbs_iinac, occ_err, olap_err))
        ...
        print("I think I have {} core inactive orbitals; occupancy error = {}, overlap error = {}".format(norbs_cinac,
                                                                                                          occ_err,
                                                                                                          olap_err))
        occ_err = linalg.norm(cno_occ[norbs_cinac:])
        olap_err = measure_basis_olap(cext2loc.conjugate().T, self.loc2imp)
        print("I think I have {} core external orbitals; occupancy error = {}, overlap error = {}".format(
            len(cno_occ) - norbs_cinac, occ_err, olap_err))
        ...
        eri_grad = np.tensordot(eri_iunac, self.twoCDMimp_amo,
                                axes=((1, 2, 3), (1, 2, 3)))  # NOTE: just saying axes=3 gives an INCORRECT result
        grad += eri_grad
        ...
        eri_grad = np.tensordot(eri_core, self.twoCDMimp_amo,
                                axes=((1, 2, 3), (1, 2, 3)))  # NOTE: just saying axes=3 gives an INCORRECT result
        grad += eri_grad
    ...
    def load_amo_guess_from_casscf_molden(self, moldenfile, norbs_cmo, norbs_amo):
        ...
        print("Difference btwn self mol coords and moldenfile mol coords: {}".format(
            sp.linalg.norm(mol.atom_coords() - self.ints.mol.atom_coords())))
    ...
    def retain_projected_guess_amo(self, loc2amo_guess):
        print("Diagonalizing fragment projector in guess amo basis and retaining highest {} eigenvalues".format(
            self.active_space[1]))
    ...
    def retain_fragonly_guess_amo(self, loc2amo_guess):
        print("Diagonalizing guess amo projector in fragment basis and retaining highest {} eigenvalues".format(
            self.active_space[1]))
        frag2amo = loc2amo_guess[self.frag_orb_list, :]
    ...
    def load_amo_guess_from_dmet_molden(self, moldenfile):
        ''' Use moldenfile of impurity from another DMET calculation with the same active space (i.e., at a different
        geometry) to guess active orbitals'''
        print("Attempting to load guess active orbitals from {}".format(moldenfile))
        mol, _, mo_coeff, mo_occ = molden.load(moldenfile)[:4]
    ...
    def align_imporbs_symm(self, imp2mo, mol=None, sort_vecs=1, sorting_metric=None, orbital_type=""):
        ...
        if self.enforce_symmetry and mol is not None:
            evals, new_imp2mo, labels = matrix_eigen_control_options(sorting_metric, subspace=imp2mo,
                                                                     symmetry=mol.symm_orb,
                                                                     sort_vecs=sort_vecs, only_nonzero_vals=False,
                                                                     strong_symm=self.enforce_symmetry)
            ...
            evals, loc2mo, labels = matrix_eigen_control_options(sorting_metric, subspace=loc2mo,
                                                                 symmetry=self.loc2symm,
                                                                 sort_vecs=sort_vecs, only_nonzero_vals=False,
                                                                 strong_symm=self.enforce_symmetry)
            err = measure_subspace_blockbreaking(loc2mo, self.loc2symm)
            new_imp2mo = self.imp2loc @ loc2mo
        labels_dict = {lbl: np.count_nonzero(labels == idx) for idx, lbl in enumerate(self.ir_names) if
                       np.count_nonzero(labels == idx) > 0}
        ...
        if molorb:
            ...
            if self.norbs_as > 0:
                try:
                    ...
                    print(idx_unac)
                    print(idx_actv)
                    print(self.norbs_imp, self.nelec_imp, self.nelec_as, loc2molden.shape, self.loc2imp.shape,
                          get_complementary_states(self.imp2amo).shape, self.loc2amo.shape)
                    raise (e)
            if canonicalize or (natorb and any([x in self.imp_solver_name for x in ('CASSCF', 'RHF')])):
                ...
                if self.norbs_as > 0:
                    ...
                    loc2molden[:, idx_actv] = matrix_eigen_control_options(metric, strong_symm=False,
                                                                           subspace=loc2molden[:, idx_actv],
                                                                           symmetry=self.loc2symm, sort_vecs=order,
                                                                           only_nonzero_vals=False)[1]
            elif natorb:
                loc2molden = \
                    matrix_eigen_control_options(oneRDM, sort_vecs=-1, only_nonzero_vals=False, symmetry=self.loc2symm,
                                                 strong_symm=False)[1]
        occ = ((oneRDM @ loc2molden) * loc2molden).sum(0)
        ene = ((FOCK @ loc2molden) * loc2molden).sum(0)
        ene[idx_actv] = 0
        if self.norbs_as > 0: ene = cas_mo_energy_shift_4_jmol(ene, self.norbs_imp, self.nelec_imp, self.norbs_as,
                                                               self.nelec_as)
    ...
    def get_rsp_1RDM_elements(self, dmet, rsp_1RDM):
        ...
        if dmet.altcostfunc:
            raise RuntimeError(
                "You shouldn't have gotten in to get_rsp_1RDM_elements if you're using the constrained-optimization cost function!")
