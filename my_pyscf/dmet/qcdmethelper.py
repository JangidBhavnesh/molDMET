...
class qcdmethelper:
    ...
    def constructbath(self, OneDM, impurityOrbs, numBathOrbs, threshold=1e-13):
        ...
        if (tokeep < numBathOrbs):
            print("DMET::constructbath : Throwing out", numBathOrbs - tokeep, "orbitals which are within", threshold,
                  "of 0 or 2.")
        ...
        for counter2 in range(0, numTotalOrbs):
            if (impurityOrbs[counter2]):
                eigenvecs = np.insert(eigenvecs, counter2, 0.0,
                                      axis=0)  # Stack rows with zeros on locations of the impurity orbitals
